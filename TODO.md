...
- [ ] **#103 — Honour `.gitignore` / `.gimmeignore` inside the uploaded archive** *(after #88, filed from it)*
  ...
  *Files:* `internal/content/content-service.go`, `internal/content/content-service_test.go`
  ⚠️ *Carries a decision on #62's limits:* `checkArchiveLimits` runs over the whole central directory before any mapping. Filtering after it keeps the limits a real bound on work; filtering before makes them a bound on output. Probably the former, but it must be stated.
  *Also to settle:* nested ignore files or root-only, which gitignore syntax subset (pick a library — `go-git`'s `plumbing/format/gitignore` or `sabhiram/go-gitignore` — do not hand-roll), whether the ignore files themselves are published (npm excludes them, and excluding them is what makes the root-detection interaction work), and what happens when the ignore file excludes everything.
