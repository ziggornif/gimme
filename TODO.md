...
- [x] **#118 — Nothing validates `docs/api/swagger.json`** *(filed mid-flight, during #50)*
  A duplicated `Gimme-Integrity` key in the `GET /gimme/{package}/{file}` 200 response killed the whole API reference page, and the full quality gate stayed green — `gofmt`, `golangci-lint`, `make test` and `make build` all passed. A duplicate key is *legal* JSON, undefined by the spec and resolved last-wins by every mainstream parser, so nothing in a Go pipeline objects. ReDoc parses the file as YAML, where it is fatal: `duplicated mapping key … (347:15)`. The only format that rejects it was the one nobody ran.
  *Files:* `redocly.yaml`, `.github/workflows/build.yml`
  *Why `@redocly/cli`:* ReDoc renders the page, so the linter shares the parser that breaks — the check fails on exactly what the page fails on. Confirmed to reproduce ReDoc's error at the same line.
  ⚠️ *The recommended ruleset does not pass on `main`* — 7 errors, 8 warnings, before this branch touched anything. All 7 errors are `security-defined` against **public** operations (`/`, `/docs`, `/gimme/{package}`, `/healthz`, `/readyz`, `/metrics`); the rule wants every operation authenticated, which is precisely wrong for a CDN. The five authenticated operations already declare security. Hence `redocly.yaml` with that rule and three style rules off.
  *This does not weaken it:* the duplicate key is a **parse** failure, not a rule violation, so it is caught with every style rule disabled. Pinned by running the job's exact command against a broken spec — exit 1 — and against `main` — exit 0.
  ⚠️ **Add `openapi` to the required status checks** on the `main` branch protection, or the job runs and blocks nothing — the same footgun called out in #52.
  *Deliberately out of scope:* the 8 style warnings (missing `operationId`s, undocumented 4xx responses, the `example.com` server URL). Real, but a spec-content change with its own review. And the duplicate-key fix itself stays on the #50 branch: it is that branch's regression, and once this check is on `main`, #50 cannot go green without it.
...
- [x] **#50 — SRI integrity hashes**
  SHA-384 per file, computed at upload, exposed as a copy-paste `<script integrity>` snippet.
  *Files:* `internal/content/content-service.go`, `internal/content/integrity.go`, `internal/storage/objectstorage-manager.go`, `test/mocks/objectstorage-manager*.go`, `api/package-controller.go`, `templates/package.tmpl`, `README.md`, `docs/site/index.html`, `docs/api/swagger.json`
  ⚠️ ***The issue as filed contradicts itself, and the measurement is what settles it.*** It prescribes S3 user metadata for storage *and* the file listing UI for display. Probed against Garage: `PutObject` + `Stat` round-trip user metadata correctly (`gimme-sri` comes back canonicalised as `Gimme-Sri` in `minio.ObjectInfo.UserMetadata`), but `ListObjects{WithMetadata: true}` returns `minio.StringMap(nil)` — `metadata=true` is a MinIO extension, not S3, and Garage ignores it. Server-rendering a hash per row therefore costs one `HEAD` per row: 31 843 requests on `@mui/icons-material@5.15.21`, on the page #84 already measures at 39.7 MB and 1.3 s.
  *So the hash is fetched lazily, on click, not rendered with the row.* An SRI button on `.js`/`.css` rows issues one `HEAD` for the file the user actually wants and builds the snippet client-side. One request instead of 31 843, and #84 gets no worse.
  *The digest is written to the variants too, and that is what makes encodings free.* SRI validates against the **decoded** body, so the identity hash is the correct one for `.br` and `.gz` alike. It is known at upload, so `AddBytes` stamps the same `gimme-sri` value on each variant — serving any encoding then emits the right digest with no extra S3 round trip, and the issue's third test holds by construction. A variant must never carry the hash of its own compressed bytes: a client would validate the wrong representation.
  *Hashing needs its own pass over the entry — a `TeeReader` cannot work.* User metadata travels as request headers, so the digest must be known **before** `PutObject` sends the body. `compressVariants` cannot carry it either: it only runs for compressible entries ≥ 1 KiB, and the hash is needed for every file. So `zip.File.Open()` once more, `io.Copy` into a `sha512.New384()`, then `AddObject`. Costs one decompression per entry — no S3 request, no buffering.
  ⚠️ *The snippet must carry the resolved pinned version, never the partial one.* A snippet built from `pkg@2.0` breaks for everyone who copied it the moment `2.0.11` is published — the URL resolves to the new patch, the hash no longer matches, the browser blocks the script, and nothing warns. This is free here: `GetFiles` puts the full object key in `File.Name`, so the row's `href` already reads `/gimme/pkg@2.0.10/…` even when the page was reached as `pkg@2.0`. The snippet takes the href, not the address bar.
  *`crossorigin="anonymous"` is part of the snippet and it needs `Access-Control-Allow-Origin`.* gimme defaults to `*`, so it works out of the box — but an operator who sets `cors.allowed_origins` to a fixed list silently breaks SRI for every cross-origin consumer: no ACAO, blocked script. Documented, not code.
  *`HEAD` is registered explicitly.* Gin binds one method per route, so `router.HEAD("/gimme/:package/*file", …)` is added alongside the `GET`, with an early return after `Stat` — reusing the GET handler would stream the object out of S3 into a writer that discards it. `cors.DefaultConfig()` already allows `HEAD`, and the listing page is same-origin, so no CORS change is needed to read the header from the button.
  *Header name:* `Gimme-Integrity`, carrying the SRI token verbatim (`sha384-<base64>`). `Digest` was rejected — RFC 3230 is obsoleted, and RFC 9530's `sha-384=:base64:` syntax would force every consumer to re-encode the value before pasting it. Reusing a registered name with different syntax is worse than an application-specific one.
  *Legacy packages degrade, they do not fail.* Objects uploaded before this change carry no metadata: the header is absent, the response is a normal 200, and the button reports the hash as unavailable rather than erroring.
  *Rollback and deletion need no change* — the metadata rides on the objects themselves, so #114's rollback and `DeletePackage`'s prefix removal already cover it.
...
- [x] **#49 — Browse: list the versions of a package**
...
- [x] **#144 — Docs site is missing what the README documents** *(plus `406` and `Content-Encoding`, absent from both)*
  ...
  *Absent from the site, present in the README:* the three `upload.*` limits and the `413` they produce, the `409` on an existing version, the partial-upload rollback, `__MACOSX` / `.DS_Store` / NFC normalisation, `GET /auth/login` and `GET /auth/callback`, `token_file`, and `.gimmeignore` beyond the pointer link #63 added. OIDC itself is covered on the site (13 mentions) — it is the two routes that are missing, not the concept.
  ⚠️ *Absent from **both**, so not a parity fix:* `406 Not Acceptable`, which `api/package-controller.go` answers when the client refuses `identity` and no stored variant matches; and `Content-Encoding` as a response header — the negotiation is described, the header never named.
  *The pagination contract is still the largest piece.* The word `limit` appears **0 times** in the 2092 lines of the site. Missing: `?limit=` (default 50, clamped `[1, 500]`), `?after=`, the `Link` header with `rel="first"`/`rel="next"`, why `rel="prev"` and `rel="last"` are **deliberately** absent (not derivable from a keyset cursor — GitLab omits the same two), the JSON body shape, and why `total` is `null` unless the listing fitted in one page.
  *The versions listing is partly there, so check before writing* — the site has a `Browse` section (`docs/site/index.html:1793`) already showing the versions JSON, and #63 added a README sentence. Genuinely absent on both sides: the semver-descending order, that a browser's `Accept` gets HTML from the same URL, and the `404` on an unknown package. That section also covers only the versions form, never `GET /gimme/<pkg>@<version>` — which is where the pagination contract belongs.
  *Checked and already covered, do not redo:* all six metrics including the S3 histogram, every caching and integrity header, `@latest`, brotli/gzip, CORS, the health routes, and 25 of the 29 config keys.
  ⚠️ *This is the rule below, unapplied.* The length of the list is the measure of how long it went unread.
