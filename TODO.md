...
- [x] **#62 — Upload limits** (size, entry count, decompressed size)
  ...
  *Default raised, 10 000 → 50 000 entries:* `@mui/icons-material@5.15.21` holds 31 843 files (the same package #84 and #85 are measured against), so the first default rejected a design-system icon package — the primary use case for a CDN. The expansion limit is what stops a zip bomb; the entry count bounds a different cost, one PUT and one permanent object per entry.
...
- [ ] **#97 — Every `c.FormFile` failure answers "input file is required"** *(found while reviewing #62)*
