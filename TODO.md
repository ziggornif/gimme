...
- [x] **#88 — Archive shapes that produce surprising object keys without erroring** *(after #42 + #43)*
  ...
  *What stays open from symptom 1:* adding a `README.md` beside a `dist/` folder still moves every asset URL. No issue filed — #55 builds the archive itself and removes the case at the source, which is the better fix. Reopen only if a hand-made zip makes it hurt in practice.
