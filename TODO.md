...
- [x] **#88 — Archive shapes that produce surprising object keys without erroring** *(after #42 + #43)*
  ...
  *The product decision on A, settled — root detection keys off a single top-level directory, and root-level files no longer block it unconditionally.* Strip when there is exactly one distinct top-level **directory** and every root-level **file** carries a known non-servable metadata name (`.gitignore`, or a base before the first dot in `README`/`LICENSE`/`LICENCE`/`CHANGELOG`, case-insensitive). Metadata files are **uploaded normally, at their own place** — the list decides only whether a root file blocks detection, it removes nothing.
  ⚠️ *The first rule considered — "one top-level directory wins, whatever sits at the root" — was rejected during implementation because it breaks a layout the test table already pinned.* `app.js` + `style.css` + `img/logo.svg` has exactly one top-level directory, so that rule would have stripped `img/` and flattened a perfectly ordinary package root with an assets folder. That is worse than symptom 2, since `img/` is visibly content. What distinguishes `dist/` from `img/` is not the directory, it is **what sits beside it at the root**: `README.md` is noise, `app.js` is content. Hence the metadata list — arbitrary by nature, small on purpose, and it touches nothing anyone would serve from a CDN.
  *Junk is dropped from the upload, not merely ignored for detection, and the matching is recursive.* `__MACOSX` as first segment, basename `.DS_Store`, basename prefixed `._` (AppleDouble files appear outside `__MACOSX/` when the zip is built from a non-HFS volume). Recursion is what the "not uploaded" half requires: the Finder writes a `.DS_Store` into **every** folder a window has opened, so a root-only rule fixes detection but still publishes `dist/.DS_Store`. This is the single deliberate exception to #42's "reject the whole archive, never skip an entry silently" rule, and it is compensated by one aggregate `logrus.Infof` line — aggregate, not per-entry, since an archive may hold tens of thousands.
